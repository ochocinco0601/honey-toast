...
4. **Hub routing** — if the perspective is reader-navigable at top level (most are), add no new row but ensure the "Or start from your platform" table in the hub README routes to the perspectives catalog.
...
| Page | Role |
|---|---|
| `README.md` | The hub — first orientation for any reader, and the full index. Renders as the site root and as the folder landing page. |
...
| Adding/removing a meta page | hub README routing, mkdocs nav, any meta page that cross-references the changed page |
...
8. **Navigation coherence check.** After updates, does the hub README's question routing still make sense, and does it still match what's reachable? Does mkdocs nav expose everything?
...
Adding/removing a meta page is itself a structural change and follows the Part VIII process for structural change. Include the full integration review: hub README routing, mkdocs nav, and any meta page that cross-references the changed page.
