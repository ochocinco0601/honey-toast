...
Pass requires the six levels — capability → intended flow → stage → process step → component → external dependency — where a flow delivers capabilities (many to many), a flow is an ordered run of stages, a stage contains its process steps, a component supports a step, and a component depends on external dependencies — the platforms, other applications and vendors the delivering application relies on and is not part of; the recorded ends are usually the top (critical products and capabilities) and the bottom (application and component inventories); the middle — flows, stages and steps — is typically unrecorded, and closing that gap is what the discipline's records exist to do.
