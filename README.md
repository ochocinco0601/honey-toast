...
| **copilot-walkthrough.md** | A run sheet for a live session that introduces GitHub Copilot to managers and other non-engineers — five questions asked of a cloned code repository, one answer run live and the rest shown from an earlier session, with a fixed clock on the setup check and a named person watching chat so nothing is debugged in front of the room. |
