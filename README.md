...
| **copilot-training-kit** | A take-home training on using GitHub Copilot inside VS Code, written for people who are not software engineers. A led first fifteen minutes, two lessons (a folder of your own files; an application you do not own), eight recipes for real chores, a reference on skills, the questions people ask with their answers, a glossary, and a presenter's run of show with its preparation list and prompt sheet. Every organization-specific answer is a marked blank to fill once. Opened as a folder in VS Code and read with the assistant's help. |
